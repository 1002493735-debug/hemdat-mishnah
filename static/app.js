...
function login(mode=false){
  ...
  view='login';nav();
  main('<section class="panel narrow intro welcome"><span class="school-logo welcome-logo"><img src="/static/assets/school-logo.jpg" alt="סמל בית הספר חמדת השקד" width="1086" height="1536"></span><p class="eyebrow">ממ״ד חמדת השקד – מבשרת ציון</p><h1>לומדים יחד<br>משלימים יחד</h1><p>כל משנה מוסיפה אור. כל אחד ואחת שותפים.</p><form id="login-form"><label for="password">'+(admin?'סיסמת מנהל':staff?'קוד הכניסה לצוות':'קוד הכניסה לתלמידים')+'</label><div class="password-field"><input id="password" type="password" autocomplete="current-password" required><button type="button" class="quiet password-toggle" data-action="toggle-password" aria-controls="password" aria-label="הצגת הקוד" aria-pressed="false"><span aria-hidden="true">👁</span><span class="toggle-label">הצגה</span></button></div><input id="role" type="hidden" value="'+(admin?'admin':staff?'staff_gate':'school')+'"><button class="cta">כניסה '+(admin?'לניהול':staff?'לצוות':'למרחב הלימוד')+'</button></form><p><button class="quiet" data-action="'+(admin?'student-login':'admin-login')+'">'+(admin?'כניסת תלמידים':'כניסת מנהל')+'</button> <button class="quiet" data-action="'+(staff?'student-login':'staff-login')+'">'+(staff?'כניסת תלמידים':'כניסת צוות')+'</button></p><p class="dedication">לעילוי נשמת הקדושים שנרצחו על קידוש ה׳ בשמחת תורה תשפ״ד, במלחמת חרבות ברזל.</p></section>');
}
...
function roundLabel(n){return 'סבב '+(['','ראשון','שני','שלישי','רביעי','חמישי','שישי','שביעי','שמיני','תשיעי','עשירי'][n]||n);}
function home(){
 const done=state.completed.length;
 main('<section class="panel intro"><p class="eyebrow">'+roundLabel(state.round.number)+'</p><h1>לומדים יחד – משלימים יחד</h1><p class="stat">🏅 סיומים בית־ספריים של שלוש המסכתות: '+state.rounds_completed+'</p><p class="dedication">הלימוד מוקדש לעילוי נשמת הקדושים שנרצחו על קידוש ה׳ בשמחת תורה תשפ״ד, במלחמת חרבות ברזל.</p><div class="row spread"><div><span class="big">'+done+'</span> מתוך '+state.total+' משניות</div><strong>'+Math.round(done/state.total*100)+'% מהמשימה הושלמה</strong></div>'+progress(done,state.total)+'<div class="row spread"><span>נותרו '+(state.total-done)+' משניות</span>'+(me?.is_staff?'<span>צוות בית הספר · לומדים יחד, ללא כרטיסי הגרלה</span>':'<span>🎟️ הכרטיסים שלי: '+state.my_tickets+'</span>')+'</div>'+(!me?.is_staff?'<p>אפשר לצבור עד 3 כרטיסים ביום. נותרו לך '+Math.max(0,3-state.daily_tickets)+' כרטיסים להיום. אפשר להמשיך ללמוד גם ללא כרטיסים! המכסה מתחדשת בחצות לפי שעון ישראל.</p>':'')+(state.round.finished_at?'<p class="stat">יחד השלמנו שלוש מסכתות!</p>':'<button class="cta" data-action="learn">📖 תנו לי משנה ללמוד</button>')+'</section><div class="grid">'+state.structure.map(t=>{const p=stats(t);return '<section class="panel">'+art(t.id)+'<h2>מסכת '+E(t.name)+'</h2><p class="stat">'+(p.done===p.all?'✓ המסכת הושלמה!':p.pct+'%')+'</p>'+progress(p.done,p.all)+'<p class="muted">'+p.done+' מתוך '+p.all+' משניות</p><button class="quiet" data-view="map">למפת המסכתות</button></section>';}).join('')+'</div>');
}
function map(){
 main('<h1>מפת המסכתות</h1><p class="stat">'+roundLabel(state.round.number)+' · '+state.rounds_completed+' סיומים בית־ספריים</p><p class="muted">✓ מסמן משנה שנלמדה בבית הספר. גם אותה אפשר ללמוד בעצמכם.</p>'+state.structure.map(t=>{
 const p=stats(t);
 ...
 if(done&&ready)return '<button class="tile done" data-learn="'+id+'" aria-label="'+E(title(id))+' נלמדה בבית הספר — אפשר ללמוד">✓</button>';
...
function showStudy(){
 ...
 main('<section class="panel study">'+art(lease.mishnah_id.split(':')[0])+'<p class="eyebrow">עוד משנה. עוד אור.</p><h1>'+E(title(lease.mishnah_id))+'</h1><div class="mishnah">'+E(lease.text)+'</div><button class="cta" data-action="question">למדתי – אפשר לשאול אותי!</button><p><button class="quiet" data-action="release">חזרה לבית שלנו</button></p></section>');
}
...
async function submitAnswer(option){
 ...
 let html;
 if(r.result==='correct'){
 html='<div class="seal">✓</div><h1>כל הכבוד! 🎉</h1><p>'+ (r.school_added?'בזכותך הושלמה משנה נוספת בבית הספר!':'למדת את המשנה בהצלחה!')+'</p><p class="stat">✓ המשנה נלמדה</p>'+(r.ticket_awarded?'<div class="ticket">זכית בכרטיס הגרלה! 🎟️</div>':'<p>'+(r.ticket_reason==='staff'?'תודה על הלימוד ועל התרומה של צוות בית הספר!':r.ticket_reason==='already_awarded'?'כבר קיבלת כרטיס על משנה זו בסבב הנוכחי. כל הכבוד על החזרה!':'צברת את 3 הכרטיסים להיום. אפשר להמשיך ללמוד, ומחר לצבור כרטיסים נוספים!')+'</p>');
 ...
 if(r.next_round)html+='<h2>🌟 '+roundLabel(r.next_round)+' נפתח! 🌟</h2>';
...
async function board(){
 ...
 main('<h1>כולנו שותפים</h1><p>כל כרטיס מספר על משנה שנלמדה. כל כיתה מוסיפה אור.</p><section class="panel"><h2>התרומה של הכיתות</h2><p class="muted">לימודים של תלמידי הכיתה בכל הסבבים יחד; כמה תלמידים יכולים ללמוד אותה משנה.</p>'+b.classes.map(c=>'<h3>'+E(c.name)+' – תרמה '+c.contribution+' משניות</h3>'+progress(c.contribution,Math.max(1,...b.classes.map(x=>x.contribution)))).join('')+'</section><section class="panel"><h2>כרטיסי הגרלה</h2><p class="muted">לפי סדר הכיתות והשמות</p>'+table(['שם','כיתה','כרטיסים'],b.students.map(s=>[E(s.name),E(s.class_name),s.tickets]))+'</section><section class="panel"><h2>פעילות אחרונה</h2>'+b.recent.map(r=>'<p>'+(r.class_name==='צוות'?'איש/אשת צוות':('תלמיד/ה מ'+E(r.class_name)))+' השלימו משנה במסכת '+names[r.tractate]+'</p>').join('')+(b.recent.length?'':'<p>המסע מתחיל במשנה הראשונה.</p>')+'</section>');
}
async function admin(){
 adminData=await api('admin/data');
 const d=adminData,settings=Object.fromEntries(d.settings.map(s=>[s.key,s.value]));
 main('<h1>ניהול המיזם</h1><div class="row"><button data-view="home">התקדמות כללית</button><button data-view="map">מפת המשניות</button><button data-view="board">כרטיסים ותרומת כיתות</button></div><section class="panel"><h2>תלמידים, צוות וכרטיסים</h2><div class="filters"><input id="search" aria-label="חיפוש תלמיד" placeholder="חיפוש תלמיד"><select id="class-filter" aria-label="סינון כיתה"><option value="">כל הכיתות</option>'+d.classes.map(c=>'<option value="'+E(c.id)+'">'+E(c.name)+'</option>').join('')+'</select></div><div id="student-table"></div><h3>תיקון כרטיס</h3><form id="ticket-form"><label for="ticket-student">תלמיד/ה</label><select id="ticket-student">'+d.students.filter(s=>!s.is_staff).map(s=>'<option value="'+E(s.id)+'">'+E(s.name)+'</option>').join('')+'</select><label for="delta">פעולה</label><select id="delta"><option value="1">הוספת כרטיס אחד</option><option value="-1">הסרת כרטיס אחד</option></select><label for="reason">סיבת התיקון</label><input id="reason" required maxlength="300"><button class="cta">שמירת תיקון</button></form></section><section class="panel"><h2>משך ניסיון לימוד</h2><p>אחרי טעות חוזרים לקריאה ולשאלה השנייה, ללא השהיה.</p><form id="settings-form"><label for="lease-seconds">משך ניסיון, בשניות</label><input id="lease-seconds" type="number" min="60" max="3600" value="'+settings.lease_seconds+'" required><button class="cta">שמירת זמנים</button></form></section><section class="panel"><h2>ייבוא ועדכון תוכן ותלמידים</h2><p>תחילה בודקים את הקובץ. לאחר בדיקה מוצלחת ניתן לשמור. ייבוא תוכן אינו משנה השלמות או כרטיסים.</p><label for="import-kind">סוג קובץ</label><select id="import-kind"><option value="content">תוכן משניות ושאלות – Excel או JSON</option><option value="students">תלמידים וכיתות – Excel, CSV או JSON</option><option value="staff">צוות – Excel, CSV או JSON (שמות בלבד)</option></select><label for="import-file">בחירת קובץ</label><input id="import-file" type="file" accept=".json,.csv,.xlsx"><label><input id="full" type="checkbox"> בדיקת מאגר תוכן מלא (149 משניות)</label><label for="import-text">תוכן הקובץ / עריכה ידנית</label><textarea id="import-text" dir="ltr" spellcheck="false"></textarea><div class="row"><button data-action="validate">בדיקת קובץ</button><button id="apply-import" data-action="import" disabled>שמירת הייבוא</button><a class="button quiet" href="/api/admin/export?kind=content">הורדת תבנית התוכן המלאה</a></div><pre id="import-report" role="status"></pre></section><section class="panel"><h2>משניות שהושלמו</h2>'+table(['משנה','תלמיד/ה','כיתה','סבב','מועד','מצב'],d.completions.map(x=>[E(title(x.mishnah_id)),E(d.students.find(s=>s.id===x.student_id)?.name||x.student_id),E(d.classes.find(c=>c.id===x.class_id)?.name||x.class_id),x.round_id,E(new Date(x.completed_at*1000).toLocaleString('he-IL')),x.revoked_at?'בוטלה':'<button class="danger" data-revoke="'+x.id+'">ביטול השלמה וכרטיס</button>']))+'</section><section class="panel"><h2>סבבים</h2>'+table(['סבב','התחלה','סיום'],d.rounds.map(r=>[r.number,E(new Date(r.started_at*1000).toLocaleString('he-IL')),r.finished_at?E(new Date(r.finished_at*1000).toLocaleString('he-IL')):'פעיל']))+'<p>לאחר כל השלמה של שלוש המסכתות נפתח הסבב הבא אוטומטית. כל ההיסטוריה נשמרת.</p><button data-action="new-round">פתיחת סבב נוסף</button></section><section class="panel"><h2>ייצוא</h2><div class="row">'+[['tickets','כרטיסים'],['raffle','רשימה מוכנה להגרלה'],['completions','השלמות והיסטוריה'],['classes','תרומת כיתות'],['students','תלמידים וכיתות'],['staff','צוות']].map(([k,n])=>'<a class="button" href="/api/admin/export?kind='+k+'">'+n+' – CSV</a>').join('')+'</div></section><section class="panel"><details><summary>יומן שינויים אחרונים</summary>'+table(['זמן','פעולה','פרטים'],d.audit.map(a=>[E(new Date(a.at*1000).toLocaleString('he-IL')),E(a.action),E(a.details)]))+'</details></section>');
 mainAppendBackup();
 studentTable();
}
function studentTable(){
 const term=$('#search').value.toLocaleLowerCase(),cls=$('#class-filter').value,d=adminData;
 $('#student-table').innerHTML=table(['שם','כיתה','כרטיסים','מצב'],d.students.filter(s=>(!cls||s.class_id===cls)&&s.name.toLocaleLowerCase().includes(term)).map(s=>[E(s.name),E(d.classes.find(c=>c.id===s.class_id)?.name),s.is_staff?'צוות – ללא כרטיסים':d.tickets.filter(t=>t.student_id===s.id).reduce((a,t)=>a+t.delta,0),'<button class="quiet" data-student="'+E(s.id)+'" data-active="'+(s.active?0:1)+'">'+(s.active?'השבתת חשבון':'הפעלת חשבון')+'</button>']));
}
...
document.addEventListener('submit',async event=>{
 event.preventDefault();const form=event.target,button=form.querySelector('button.cta');if(button)button.disabled=true;notify();
 try{
...
document.addEventListener('click',async event=>{
 ...
 case 'toggle-password':{const input=$('#password'),show=input.type==='password';input.type=show?'text':'password';b.setAttribute('aria-pressed',String(show));b.setAttribute('aria-label',show?'הסתרת הקוד':'הצגת הקוד');b.querySelector('.toggle-label').textContent=show?'הסתרה':'הצגה';break;}
