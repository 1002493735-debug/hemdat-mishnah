...
async function submitAnswer(option){
 ...
 let html;
 if(r.result==='correct'){
 html='<div class="seal">✓</div><h1>כל הכבוד! 🎉</h1><p>בזכותך הושלמה משנה נוספת!</p><p class="stat">✓ המשנה נלמדה</p>'+(r.ticket_awarded?'<div class="ticket">זכית בכרטיס הגרלה! 🎟️</div>':'<p>תודה על הלימוד ועל התרומה של צוות בית הספר!</p>');
...
async function board(){
 ...
 main('<h1>כולנו שותפים</h1><p>כל כרטיס מספר על משנה שנלמדה. כל כיתה מוסיפה אור.</p><section class="panel"><h2>התרומה של הכיתות</h2><p class="muted">בכל הסבבים יחד</p>'+b.classes.map(c=>'<h3>'+E(c.name)+' – תרמה '+c.contribution+' משניות</h3>'+progress(c.contribution,Math.max(1,...b.classes.map(x=>x.contribution)))).join('')+'</section><section class="panel"><h2>כרטיסי הגרלה</h2><p class="muted">לפי סדר הכיתות והשמות</p>'+table(['שם','כיתה','כרטיסים'],b.students.map(s=>[E(s.name),E(s.class_name),s.tickets]))+'</section><section class="panel"><h2>פעילות אחרונה</h2>'+b.recent.map(r=>'<p>'+(r.class_name==='צוות'?'איש/אשת צוות':('תלמיד/ה מ'+E(r.class_name)))+' השלימו משנה במסכת '+names[r.tractate]+'</p>').join('')+(b.recent.length?'':'<p>המסע מתחיל במשנה הראשונה.</p>')+'</section>');
}
