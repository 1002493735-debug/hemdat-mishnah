...
function art(t){return '<img class="art" src="/static/assets/'+t+'.png" alt="'+({yoma:'בית המקדש באור של בוקר',sukkah:'סוכה וארבעת המינים','rosh-hashanah':'שופר ואווירת ראש השנה'}[t])+'" width="600" height="400">';}
function home(){
...
async function question(){
 ...
 main('<section class="panel study"><p class="eyebrow">'+E(title(lease.mishnah_id))+'</p><h1>'+E(q.prompt)+'</h1><div class="options">'+q.options.map(o=>'<button data-answer="'+o.id+'">'+E(o.text)+'</button>').join('')+'</div></section>');
}
...
document.addEventListener('change',async event=>{
 ...
 $('#import-text').value=await file.text();$('#apply-import').disabled=true;
 }
