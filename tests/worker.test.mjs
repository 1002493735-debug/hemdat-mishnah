...
let mf,db;
before(async()=>{mf=new Miniflare({modules:true,scriptPath:'dist/server/index.js',compatibilityDate:'2025-09-06',d1Databases:['DB'],bindings:{ADMIN_PASSWORD:'test-only-password',SCHOOL_CODE:'test-only-code'},serviceBindings:{ASSETS:()=>new Response('asset')}});db=await mf.getD1Database('DB');for(const file of (await fs.readdir('drizzle')).filter(f=>f.endsWith('.sql')).sort()){for(const stmt of (await fs.readFile('drizzle/'+file,'utf8')).split('--> statement-breakpoint').filter(s=>s.trim()))await db.prepare(stmt).run();}});
...
test('final completion opens bonus without losing history; later rounds supported',async()=>{const c=await login('demo-3'),x=await correct(c,'yoma:8:9');const rows=await db.prepare('SELECT id FROM mishnayot WHERE id<>? AND id NOT IN(SELECT mishnah_id FROM completions WHERE round_id=1 AND revoked_at IS NULL)').bind(x.l.mishnah_id).all();for(const r of rows.results)await db.prepare("INSERT INTO completions(round_id,mishnah_id,student_id,class_id,completed_at,content_revision) VALUES(1,?,'demo-4','demo-b',unixepoch(),1)").bind(r.id).run();const result=await c.api('answer',{id:x.l.id,option:x.option});assert.equal(result.data.bonus_round,2);assert.equal((await db.prepare('SELECT COUNT(*) n FROM completions WHERE round_id=1 AND revoked_at IS NULL').first()).n,149);const state=(await c.api('state')).data;assert.equal(state.round.number,2);assert.equal(state.completed.length,0);const admin=client();await admin.api('login',{role:'admin',password:'test-only-password'});assert.equal((await admin.api('admin/round',{})).status,400);await db.prepare('UPDATE rounds SET finished_at=unixepoch() WHERE number=2').run();assert.equal((await admin.api('admin/round',{})).data.number,3);});
